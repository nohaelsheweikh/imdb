...
export class Low extends React.PureComponent {
    state = {
      data:[],
      themeColor:''
    };
    componentWillReceiveProps(nextProps) {
      if (nextProps.low !== this.props.low) {
        this.setState({
          data: nextProps.low
        });
      }
   ...
   componentDidMount() {
     this.setState({data:this.props.low})
   }
    ...
    <View key = {item.id} style={styles.cardContainer}>
      <RkCard style={styles.Card}> 
        <View style={{flexDirection:'row',justifyContent:'space-between'}}>
            <View rkCardHeader>
                <Text style={styles.LowHearderText}>{item.code}</Text>
            </View>
            <View style={styles.date} rkCardHeader>
